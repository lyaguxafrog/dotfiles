...
mkdir ~/Videos
 ...
 git clone --depth 1 --recurse-submodules https://github.com/lyaguxafrog/dotfiles ~/Documents/dotfiles

# packages
yay -S xorg-server xorg-xrandr xorg-xrdb rsync psmisc dunst nitrogen openbox rofi rxvt-unicode-truecolor-wide-glyphs tint2 picom obmenu-generator perl-gtk3 pipewire lib32-pipewire pipewire-pulse pipewire-alsa helvum mpd mpc ncmpcpp alsa-utils brightnessctl imagemagick scrot w3m wireless_tools xclip xsettingsd xss-lock thunar thunar-archive-plugin thunar-volman ffmpegthumbnailer tumbler inkscape mpv parcellite pavucontrol viewnior xfce4-power-manager htop neofetch fish
...
sudo cp EXTRA_JOYFUL/30-touchpad.conf /etc/X11/xorg.conf.d/30-touchpad.conf
